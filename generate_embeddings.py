...
for application_number, text in rows:
    try:
        ...
        # Extract IDEA SUMMARY section for embedding (optional use case)
        summary_start = report.find("STEP 1")
        summary_text = report[summary_start: summary_start + 1500] 
        ...
        """, (text, embedding_data, application_number))

        conn.commit()
        print(f"Embedded and analyzed patent {application_number}")

    except Exception as e:
        print(f"Error processing {application_number}: {e}")
