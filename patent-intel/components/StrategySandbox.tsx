...
const StrategySandbox: React.FC<StrategySandboxProps> = ({ data, onNavigate }) => {
  ...
  const metrics = useMemo(() => {
    ...
    const totalWeight = activeFeatures.reduce((acc, f) => acc + statusWeights[f.status], 0);
  ...
  const handleShuffle = () => {
    ...
    setFeatures(prev => prev.map((f, idx) => {
      switch (nextProfile) {
        case 'Aggressive': return { ...f, status: 'unique', enabled: true };
        case 'Risk-Averse': return { ...f, status: f.status === 'high-risk' ? 'standard' : f.status, enabled: true };
        case 'Minimalist': return { ...f, enabled: idx === 0 };
        case 'Maximum-Breadth': return { ...f, status: 'partial', enabled: true };
        case 'Disruptive': return { ...f, status: Math.random() > 0.5 ? 'unique' : 'high-risk', enabled: Math.random() > 0.3 };
        case 'Defensive': return { ...f, status: 'standard', enabled: true };
        default: return { ...f, status: data.features[idx].status, enabled: true };
      }
    }));
  };
      ...
      <div className="grid grid-cols-12 gap-8 items-start">
        ...
        <div className="col-span-12 lg:col-span-5 space-y-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            {/* Novelty Probability Widget - IMPROVED UI */}
            ...
            {/* Grant Confidence Widget - IMPROVED UI (Segmented Bar) */}
                        ...
                        <div className="text-[15px] text-slate-600 leading-[1.8] font-medium">
                          {activeFeatures.length > 0 ? (
                            <p>
                              ...
                              The invention utilizes {activeFeatures.length > 1 ? `<span class="text-slate-900 font-bold">${activeFeatures[1].name.toLowerCase()}</span>` : 'adaptive coordination logic'} to 
