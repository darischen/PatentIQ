...
import { User, Mail, Shield, Award, Calendar, FileText, Zap, ChevronRight, Edit3, Grid, Camera } from 'lucide-react';
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 max-w-5xl mx-auto">
      ...
      <div className="bg-white rounded-[3rem] shadow-xl border border-slate-50 overflow-hidden">
        <div className="h-40 bg-gradient-to-r from-indigo-600 via-indigo-500 to-purple-500 relative">
          <button className="absolute bottom-6 right-10 bg-white/20 hover:bg-white/30 backdrop-blur-md text-white px-5 py-2.5 rounded-full text-xs font-bold flex items-center gap-2 transition-all">
        ...
        <div className="px-12 pb-12 relative">
          <div className="flex justify-between items-end">
             <div className="relative -mt-16 mb-4">
                ...
                <button className="absolute bottom-1 right-1 w-10 h-10 bg-indigo-600 text-white rounded-full flex items-center justify-center border-4 border-white shadow-lg hover:scale-105 active:scale-95 transition-all">
                  <Camera size={16} />
                </button>
             </div>
             
             <div className="flex gap-4 mb-4">
               <button className="bg-slate-900 text-white px-8 py-3.5 rounded-xl font-black text-[11px] uppercase tracking-widest flex items-center gap-2 hover:bg-slate-800 transition-all active:scale-95 shadow-xl shadow-slate-200">
                 Edit Profile
               </button>
               <button onClick={() => onNavigate('settings')} className="bg-white border border-slate-100 text-slate-500 p-3.5 rounded-xl hover:bg-slate-50 transition-all active:scale-95">
                 <Grid size={18} />
               </button>
             </div>
          </div>
        ...
        <div className="col-span-12 lg:col-span-5 space-y-8">
           ...
           <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm space-y-6">
              ...
              <div className="pt-4 px-2">
                <button onClick={() => onNavigate('history')} className="w-full py-4 bg-slate-900 text-white rounded-xl font-black text-[11px] uppercase tracking-widest hover:bg-slate-800 transition-all flex items-center justify-center gap-2">
